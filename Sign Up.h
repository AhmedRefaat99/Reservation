...
namespace Reservation {
	...
	{
	public:
		System::Windows::Forms::Form ^start;
		SignUp(void)
		{
			InitializeComponent();
			//
		...
		SignUp(System::Windows::Forms::Form ^ obj1)
		{
			start = obj1;
			InitializeComponent();
			//
...
#pragma region Windows Form Designer generated code
		...
		void InitializeComponent(void)
		{
			...
			this->label2->Click += gcnew System::EventHandler(this, &SignUp::label1_Click);
			// 
			// textBox2
			// 
			...
			this->label4->Location = System::Drawing::Point(174, 56);
			this->label4->Name = L"label4";
			this->label4->Size = System::Drawing::Size(626, 55);
			this->label4->TabIndex = 5;
			this->label4->Text = L"Please Sign up to continue";
			// 
			// label3
			// 
...
#pragma endregion
	...
	private: System::Void button1_Click(System::Object^  sender, System::EventArgs^  e) {
			...
			string user = msclr::interop::marshal_as<std::string>(username);
			string pass = msclr::interop::marshal_as<std::string>(password);

			if ( user == "" || user.find_first_not_of(' ') == std::string::npos)
				MessageBox::Show("Please Enter Username. ");
			else if ( pass == "" || pass.find_first_not_of(' ') == std::string::npos)
				MessageBox::Show("Please Enter Password. ");
			else 
			{
				signup( username, password );
				this->Close();
				MainPage^mainpage = gcnew MainPage(start);
				mainpage->Show();
			}
		}
